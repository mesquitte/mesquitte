...
macro_rules! encodable_packet {
    ($typ:ident($($field:ident),* $(,)?)) => {
        impl $crate::common::packet::EncodablePacket for $typ {
            type Output = $crate::v4::control::fixed_header::FixedHeader;
...
mod codec {
    ...
    use crate::{
        ...
        v4::control::packet_type::{PacketType, PacketTypeError},
    };
...
mod test {
    ...
    async fn test_variable_packet_framed() {
        ...
        let sub_packet = SubscribePacket::new(
            1,
            vec![(TopicFilter::new("foo/#").unwrap(), QualityOfService::Level0)],
        );
